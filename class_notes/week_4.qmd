...
We could break this into a few parts for our own comfort. Let's start by just creating a function that grabs the data. As that's the one that's a time intensive, and calls to the API, let's try to run it only once.
```{r}
...
We can start by putting this all into a super quick and ineffcient function like this (hoping to improve it as we go along):
...
```{r}
key_plotter("1yYlpGuBiRRf33e1gY61bN", indie_major_key, indie_minor_key)
```